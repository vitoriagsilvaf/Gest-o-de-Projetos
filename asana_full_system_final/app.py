...
def criar_projeto():
    ...
    db.session.add(novo)
    db.session.commit()
    db.session.add(Historico(projeto_id=novo.id, status='pendente'))
    db.session.commit()
...
def mover_projeto(projeto_id, novo_status):
    ...
    if status_anterior != 'andamento' and novo_status == 'andamento':
        tempo = Tempo(projeto_id=projeto.id, inicio=datetime.now())
        db.session.add(tempo)
    elif status_anterior == 'andamento' and novo_status != 'andamento':
        tempo = Tempo.query.filter_by(projeto_id=projeto.id, fim=None).order_by(Tempo.inicio.desc()).first()
        if tempo:
            tempo.fim = datetime.now()
...
def ver_projeto(projeto_id):
    ...
    tempos = Tempo.query.filter_by(projeto_id=projeto.id).all()

    tempo_total = 0
    for t in tempos:
        if t.fim:
            tempo_total += (t.fim - t.inicio).total_seconds()
        else:
            tempo_total += (datetime.now() - t.inicio).total_seconds()

    horas = int(tempo_total // 3600)
    minutos = int((tempo_total % 3600) // 60)
    segundos = int(tempo_total % 60)

    return render_template('ver_projeto.html', projeto=projeto, historico=historico, tempo=f"{horas:02}:{minutos:02}:{segundos:02}")
...
def editar_projeto(projeto_id):
    ...
    if request.method == 'POST':
        ...
        nova_desc = request.form['descricao']
        if projeto.nome != novo_nome:
            db.session.add(Historico(projeto_id=projeto.id, status='✏️ Nome alterado'))
        if projeto.descricao != nova_desc:
            db.session.add(Historico(projeto_id=projeto.id, status='📝 Descrição atualizada'))
        projeto.nome = novo_nome
        projeto.descricao = nova_desc
        db.session.commit()
...
def excluir_projeto(projeto_id):
    ...
    Tempo.query.filter_by(projeto_id=projeto.id).delete()
